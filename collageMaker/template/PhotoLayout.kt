package com.cooptech.collagephotoeditor.collageMaker.template
...
import com.cooptech.collagephotoeditor.collageMaker.model.ImageTemplate
...
class PhotoLayout : RelativeLayout, ItemImageView.OnImageClickListener {
    ...
    private fun asyncCreateBackgroundImage(path: String?) {
        ...
        // Launch a coroutine in the IO context for background work
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // Perform the background task in IO thread
                val result: Bitmap? = path?.let {
                    var image = ImageDecoder.decodeFileToBitmap(it)
                    if (image != null) {
                        val blurredImage = PhotoUtils.blurImage(image, 10f)
                ...
                withContext(Dispatchers.Main) {
                    ...
                    result?.let {
                        backgroundImageView?.init(it, mViewWidth, mViewHeight, mOutputScaleRatio)
                    }
                }
